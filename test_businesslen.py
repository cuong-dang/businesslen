...
class TestVerifyInit(unittest.TestCase):
    ...
    def test_valid_period(self):
        ...
        self.assertEqual(main._verify_init(start_dt, end_dt, None, 12, None),
                         (False, 1))
        # end_dt < start_dt
    ...
    def test_valid_workweek_schedule(self):
        ...
        self.assertEqual(
            main._verify_init(datetime.now(), datetime.now(),
                              TCB_WORKWEEK_SCHEDULE, None, ()),
            (False, 4)
            )
        self.assertEqual(
            main._verify_init(datetime.now(), datetime.now(),
                              TCB_WORKWEEK_SCHEDULE, 13, ()),
            (True, 0)
            )
    ...
    def test_valid_offdays(self):
        ...
        self.assertEqual(
            main._verify_init(datetime.now(), datetime.now(),
                              main._DEFAULT_WORKWEEK_SCHEDULE, 12, None),
            (False, 3)
            )
        # invalid country code
        self.assertEqual(
            main._verify_init(datetime.now(), datetime.now(),
                              main._DEFAULT_WORKWEEK_SCHEDULE, 12, "0"),
            (False, 3)
            )
        ...
        self.assertEqual(
            main._verify_init(datetime.now(), datetime.now(),
                              main._DEFAULT_WORKWEEK_SCHEDULE, 12,
        ...
        self.assertEqual(
            main._verify_init(datetime.now(), datetime.now(),
                              main._DEFAULT_WORKWEEK_SCHEDULE, 12, "US"),
            (True, 0)
            )
        # valid datetime list
        self.assertEqual(
...
class TestCore(unittest.TestCase):
    ...
    def test_calculate_work_hours(self):
        ...
        bl = main.BusinessLen(sh, eh, TCB_WORKWEEK_SCHEDULE, 12, offdays)
        ...
        bl = main.BusinessLen(sh, eh, TCB_WORKWEEK_SCHEDULE, 12, offdays)
        self.assertEqual(round(bl.hours, DEFAULT_DECIMAL_PRECISION),
